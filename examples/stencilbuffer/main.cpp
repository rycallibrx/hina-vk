...
static bool example_init(hina_example_app* app) {
  ...
  g_app.ubo_mapped = static_cast<UniformData*>(hina_mapped_buffer_ptr(g_app.ubo));
    if (!g_app.ubo_mapped) {
    ...
    toon_pip_desc.color_formats[0] = hina_get_surface_format();
    ...
    outline_pip_desc.color_formats[0] = hina_get_surface_format();
